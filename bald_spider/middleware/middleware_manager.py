...
from bald_spider.execptions import MiddlewareIntError, InvalidOutput, RequestMethodError, IgnoreRequest, NotConfigured
...
from bald_spider.event import ignore_request, response_receive
...
class MiddlewareManager:
        ...
        for method in reversed(self.methods['process_response']):
            try:
                ...
                if isinstance(response, Response):
                    continue
                raise InvalidOutput(f"{method.__qualname__} must return Request or Response, got {type(response).__name__}")
        return response
    ...
    async def _process_exception(self, request: Request, exc: Exception):
        for method in reversed(self.methods['process_exception']):
            ...
            if response is True:
                break
            raise InvalidOutput(f"{method.__qualname__} must return None or Request or Response or True, got {type(response).__name__}")
        else:
            raise exc
    ...
    async def download(self,request: Request) -> Optional[Response]:
        ...
        try:
            ...
            create_task(self.crawler.subscriber.notify(response_receive, response, self.crawler.spider))
        if isinstance(response, Response):
            response = await self._process_response(request, response)
        if isinstance(response, Request):
            await self.crawler.engine.enqueue_request(response) #可能错误
            return None
        return response
